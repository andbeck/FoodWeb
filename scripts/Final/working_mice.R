...
meth["logM"] <- "pmm"
meth["logN"] <- "pmm"
meth["logB"] <- "~I(logM + logN)" # addition of logs is the same as multiplying exponent of log
pred[c("logM", "logN"), "logB"] <-  0

c_imputed <- 
  mice(imp_csm, meth = meth, predictorMatrix = pred, m = 1, maxit = 50)

bind_me <- complete(c_imputed)

nodes <- 
  bind_cols(bind_me, nodes_imputation)
...
- sum(rowSums(mat.fluxes[consumer.type == "micropredator" ,]))
