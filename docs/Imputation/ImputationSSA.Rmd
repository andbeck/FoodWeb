...
knitr::opts_chunk$set(echo = TRUE,message=FALSE,results = "asis", warning = FALSE)
```
...
Using the imputation function `mice()` we can now impute the missing data. the `m` value corresponds to the number of data sets imputed and for this example we will use 5 as it's easier to show, but for real analysis more appropriate iterations of imputation are suggested (50+). The data will be imputed using the `pmm` method whcih is short for predictive mean matching.
```{r, results = FALSE}
